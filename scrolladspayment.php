...
if(isset($_POST[submit]))
{
	...
	$sql="INSERT INTO payment(advertiserid,transaction_type,paymenttype,paidamt,cgst,sgst,paymentdate,note,status)VALUES('$_SESSION[advertiserid]','ScrollerAdvertisement','$_POST[paymenttype]','$_POST[videoadvtcost]','$_POST[cgst]','$_POST[sgst]','$dt','Card holder - $_POST[cardholder] | Card No. -  $_POST[cardno] | Expiry date - $_POST[expdate] | CVV No. $_POST[cvvno]','Active')";	
	...
	{			
		...
		$sqladvtorder = "INSERT INTO advtorder(paymentid, tvprogramid, advertiserid,  scrolleradvtid,  fdate,  cost, costperclick, note, status) VALUES ('$insid','$rsadsprogram[tvprogramid]','$_SESSION[advertiserid]','$rsadsprogram[scrolleradvtid]','$dt','$_POST[videoadvtcost]','0','$_POST[note]','Active')";
...
<div class="col-md-12 skel-cell-important">
	<header>
		<h3>Make payment for SCROLLER ADS	</h3>
	</header>
...
<br>				
	...
	/* */  ?>%)
		...
		Card Holder<span id="idcardholder" class="errmsg"></span><input type="text" class="form-control" name="cardholder" id="cardholder" value="<?php echo $rsedit[cardholder]; /* */  ?>" placeholder="Card Holder" >
		
		Card Number <span id="idcardno" class="errmsg"></span><input type="text" class="form-control" name="cardno" id="cardno" value="<?php echo $rsedit[cardno]; /* */  ?>" placeholder="Card Number" >
		
		Expiry Date <span id="idexpdate" class="errmsg"></span><input type="month" class="form-control" name="expdate" id="expdate" value="<?php echo $rsedit[expdate]; /* */  ?>" min="<?php echo date("Y-m"); /* */  ?>" placeholder="Expiry Date"  >
		
		CVV Number <span id="idcvvno" class="errmsg"></span><input type="text" pattern="^[0-9]{3}" title="only 3 digits" required class="form-control" name="cvvno" id="cvvno" value="<?php echo $rsedit[cvvno]; /* */  ?>" placeholder="CVV Number" >
...
function validateform()
{
	 ...
	 {
		 document.getElementById("paymenttype").innerHTML = "Payment type should not be empty..";
		 checkcondition = "false";
	 }
	 if(document.frmwebadspayment.cardholder.value=="")
	 {
		 document.getElementById("cardholder").innerHTML = "Card holder name should not be empty..";
		 checkcondition = "false";
	 }
	 if(document.frmwebadspayment.cardno.value=="")
	 {
		 document.getElementById("cardno").innerHTML = "Card number should not be empty..";
		 checkcondition = "false";
	 }
	 if(document.frmwebadspayment.cvvno.value=="")
	 {
		 document.getElementById("cvvno").innerHTML = "CVV Number should not be empty..";
		 checkcondition = "false";
	 }
	 if(document.frmwebadspayment.expdate.value=="")
	 {
		 document.getElementById("expdate").innerHTML = "Mension the Expiry date";
		 checkcondition = "false";
	 }
	  if(document.frmwebadspayment.note.value=="")
	 {
		 document.getElementById("note").innerHTML = "Note should not be empty";
		 checkcondition = "false";
	 }
